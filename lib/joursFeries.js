 ...
 */
function joursFeries(year, options = { zone: 'metropole' }) {
...
function getWeekJoursFeries(date = new Date(), joursFs = undefined) {
  console.log(date, joursFs);
  if (!joursFs) {
...
function isSameWeek(date1, date2, excludeWeekEnd = true) {
  const dayOfWeekDate1 = date1.getDay();
  const dayOfWeekDate2 = date2.getDay();
  if (excludeWeekEnd && (dayOfWeekDate1 === 0 || dayOfWeekDate1 > 5 || dayOfWeekDate2 === 0 || dayOfWeekDate2 > 5)) {
    return false;
  }
